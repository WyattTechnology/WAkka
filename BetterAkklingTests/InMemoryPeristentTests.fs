module BetterAkklingTests.InMemoryPeristentTests
...
let ``spawn with name`` () =
    TestKit.testDefault <| fun tk ->
        ...
        let act = Better.spawn tk.Sys (Better.Props.Named "test") (Better.InMemoryPersistent <| handle ())
...
let ``spawn with no name`` () =
    TestKit.testDefault <| fun tk ->
        ...
        let act = Better.spawn tk.Sys Better.Props.Anonymous (Better.InMemoryPersistent <| handle ())
...
let ``create actor can create an actor`` () =
    TestKit.testDefault <| fun tk ->
        ...
        let act : IActorRef<Msg> =
            Better.spawn tk.Sys (Better.Props.Named "test") (Better.InMemoryPersistent <| handle ())
...
let ``unstash one only unstashes one message at a time`` () =
    TestKit.testDefault <| fun tk ->
        ...
        let act = Better.spawn tk.Sys (Better.Props.Named "test") (Better.InMemoryPersistent <| handle false)
...
let ``watch works`` () =
    TestKit.testDefault <| fun tk ->
        ...
        let watched = Better.spawn tk.Sys (Better.Props.Named "watched") (Better.InMemoryPersistent <| otherActor ())
        ...
        let _act = Better.spawn tk.Sys (Better.Props.Named "test") (Better.InMemoryPersistent start)
...
let ``unwatch works`` () =
    TestKit.testDefault <| fun tk ->
        ...
        let act = Better.spawn tk.Sys (Better.Props.Named "test") (Better.InMemoryPersistent start)
...
let ``crash handler is invoked if actor crashes`` () =
    TestKit.testDefault <| fun tk ->
        ...
        let start = Better.actor {
            let! crasher =
                Actions.createChild (fun f ->
                    Better.spawn f (Better.Props.Named "crasher") (Better.InMemoryPersistent crashStart)
                )
            typed probe <! crasher
            return! handle ()
        }
        ...
        let _parent = Better.spawn tk.Sys parentProps (Better.InMemoryPersistent start)
